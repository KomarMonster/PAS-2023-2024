...
print("[%s] TCP ECHO (fixed-length messages) Server is waiting for incoming connections ... " % strftime(
    "%Y-%m-%d %H:%M:%S", gmtime()))
        ...
        while True:
            try:
                ...
                if data:
                    ...
                    print(
                        "[%s] Client %s disconnected ... " % (strftime("%Y-%m-%d %H:%M:%S", gmtime()), client_address))
                    break
